...
**터틀 S1 / S2 신고가 돌파 + 시간 가드 안착** — 두 조건을 **AND** 로 모두 만족해야 진입한다.

- 직전 20일(S1) 또는 55일(S2) 신고가를 현재가가 돌파 (`turtle_s1/s2_signal = True`)
- 돌파 후 **1시간(60분) 동안 시간 가드**가 구동되며, 가드 시간 도중 `돌파선 - 0.5 * ATR` 마지노선 붕괴 없이 1시간 만료 시점에 `현재가 ≥ 돌파선` 일 때 진입 (`turtle_s1/s2_entry_ready = True`)

**상태 흐름 (target_manager → unheld_coin_record.json 에 저장):**
- 최초 돌파 시 타이머 및 마지노선 작동 시작.
- 1시간 대기 도중 마지노선 붕괴 또는 만료 시 돌파선 아래면 타이머 리셋 및 신호 삭제.
- 타이머 리셋 시, 다음 돌파 감시 목표가를 `최고가 + 0.2 * ATR`로 상향 갱신하여 횡보장 루프 차단.
