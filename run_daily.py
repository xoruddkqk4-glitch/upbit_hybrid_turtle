...
#
# 역할:
#   Google Sheets 포트폴리오 스냅샷·손익차트처럼
#   하루에 한 번(장 마감 직전)만 돌아가면 충분한 작업들을 모아 실행한다.
#   crontab 등의 스케줄러가 KST 23:55 에 1회만 이 스크립트를 호출한다.
#
# 실행 순서:
#   1. Upbit 로그인
...
#   - ATR 캐시 갱신은 run_cache.py (KST 09:10) 가 담당한다.
#     run_all.py 는 하루 종일 캐시를 읽기만 한다.
#
# 실행 방법:
#   python run_daily.py
...
def main():
    ...
    try:
        ...
        print(msg)
        SendMessage(msg)
    _step_done(t, "STEP 2: 포트폴리오 추이 기록")

    # ─────────────────────────────────────
    # STEP 3: 실현 손익 차트 갱신
    #   '포트폴리오 추이' 시트의 실현손익 열을 소스로 사용
    # ─────────────────────────────────────
    t = _step_start("STEP 3: 실현 손익 차트")
    try:
