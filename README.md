...
고전 터틀 트레이딩의 **자금관리 원칙**에 **동적 목표가 + 30분 가드** 진입 필터를 결합한 하이브리드 전략을 구현한다.
...
- **이중 진입 필터**: `max(현재가 × 1.02, 240분봉 20MA × 1.005)` 돌파 후 **30분 연속 유지** 될 때만 진입
- **터틀 오리지널 S1/S2 돌파**: 20일 / 55일 신고가 돌파 시 즉시 진입(보조 경로)
- **리스크 균등화 수량**: `1 Unit = 총자본 × 2% / ATR(N)` → 코인별 리스크 동일화
- **0.5N 피라미딩**: 마지막 매수가 대비 0.5N 상승 시마다 1 Unit 추가 (코인당 최대 4 Unit)
...
로그인 → 손절·익절 감시 → 목표가 갱신 → 30분 가드 체크 → 주문 순서대로 **1회** 실행하고 종료한다.
...
# ─── 포트폴리오 스냅샷 + 실현 손익 차트 (KST 09:40, 1일 1회) ───
40 9 * * * cd $PROJECT && $PY run_daily.py >> $PROJECT/cron_daily.log 2>&1
...
> **동작 설명**  
> - 매 10분 (`run_all.py`): 손절·익절 감시 → **목표가·일봉 지표 갱신** → 30분 가드 체크 → 신규 진입/피라미딩. 일봉 기반 지표도 이 사이클에서 함께 갱신되므로 하루 한 번만 갱신되는 게 아니라 다음 10분 사이클에 자연 반영된다.  
> - 매일 09:40 (`run_daily.py`): `포트폴리오 추이` 시트에 누적 실현손익 1점 기록 + `손익차트` 시트·차트 갱신. 이 잡은 **주문을 내지 않는다**. 이미 당일 매도로 행이 생성된 경우 해당 행을 최신값으로 덮어쓴다(중복 없음).  
> - 두 잡이 09:40·09:50 에 붙어 실행돼도 서로 다른 파일에 쓰므로(`held_coin_record.json`·`trade_ledger.json` 은 한쪽만 변경) 충돌이 없다. 원장은 append-only 라 동시 쓰기도 안전하다.  
...
> `cron.log` / `cron_daily.log` 는 자동 로테이션이 없으므로 한 달에 한 번 정도 `truncate -s 0 cron.log` 또는 `logrotate` 로 비우는 것을 권장한다.
...
두 경로 중 하나만 만족하면 진입한다. 같은 코인이 여러 경로에 해당하면 **S2 > S1 > TARGET_30MIN** 순으로 하나만 선택한다.

**A. 동적 목표가 + 30분 가드 (`TARGET_30MIN`)**

```
pending_target = max(
    현재가 × 1.02,         # 단기 2% 돌파
    240분봉 20MA × 1.005   # 중기 이평 안착
)
```

- 현재가가 `pending_target` 이상에서 **30분 연속 유지** 시 1차 Unit 매수
- 중간에 이탈하면 타이머(`above_target_since`) 초기화
- 가격이 `reference_price` 아래로 내려가면 목표가 하향 조정

**B. 터틀 S1 / S2 신고가 돌파 (`TURTLE_S1` / `TURTLE_S2`)**

- 직전 20일(S1) 또는 55일(S2) 장중 고가를 현재가가 돌파하면 **30분 가드 없이 즉시 진입**
- 강한 추세 초입을 놓치지 않기 위한 보조 경로

### 포지션 사이징 — 단계별 진입 구조

```
이론 1U 수량 = (총 자본 × 0.01) / ATR(N)        ← 터틀 정석(1% 리스크)
이론 1U 명목가 = 이론 1U 수량 × 현재가
tier_ratio = 이론 1U 명목가 / 총자본
```

`tier_ratio` 값에 따라 4단계로 진입 유형을 분류한다:

| tier_ratio | 진입 단계 | 적용 수량 | max_unit | 코인당 최대 비중 |
|---|---|---|---|---|
| ≤ 10% | **NORMAL** | 이론 수량 그대로 | 4 | ≤ 40% |
| 10% ~ 25% | **CAPPED** | 자본 10% 로 축소 | 4 | 40% |
| 25% ~ 50% | **EXCEPTION** | 자본 10% 로 축소 | 2 | 20% |
| > 50% | **SKIP** | — | — | 진입 불가 |

- **ATR(N)**: 최근 20일 True Range 평균. 코인별 변동성에 반비례해 수량 결정.
- 2N 손절까지 이탈 시 **최대 손실 = 자본의 2%** (정석 터틀, NORMAL 기준)
- **CAPPED/EXCEPTION**: 수량을 줄이므로 실제 2N 손실이 2% 보다 작아져 더 보수적.
- **EXCEPTION**: 변동성이 너무 낮아 1U 가 자본의 25% 를 넘는 코인. 피라미딩을 2U 로 제한해 비중 집중 방지 (LS 버전의 "예외 진입" 과 동일 철학).
- **SKIP**: 자본이 너무 작아 이 코인에서는 터틀 전략이 작동 불가. 자본을 키우거나 감시목록에서 제외.
- 상수는 `turtle_order_logic.py` 에서 조정 가능:
  - `RISK_PER_TRADE = 0.01` (트레이드당 리스크 1%)
  - `NORMAL_TIER_PCT = 0.10` (정상 구간 경계)
  - `CAPPED_TIER_PCT = 0.25` (CAPPED 경계)
  - `EXCEPTION_TIER_PCT = 0.50` (EXCEPTION 경계 — 초과 시 SKIP)
  - `EXCEPTION_MAX_UNIT = 2` (EXCEPTION 피라미딩 상한)
- 자본이 커지면 대부분 코인이 자연스럽게 NORMAL 구간으로 진입 (LS 버전과 동일).
- `수량 × 현재가 < 5,000원` 이면 Upbit 최소 주문 제한으로 스킵.

### 피라미딩

- **0.5N 상승 시마다 1 Unit 추가**
- **코인당 최대 4 Unit**, **포트폴리오 전체 12 Unit** 상한
...
│   ├── target_manager.py    — 목표가 계산, unheld_coin_record.json 관리
│   └── timer_agent.py       — 30분 가드 + 터틀 S1/S2 통합 신호 산출
...
4. **목표가 갱신** — 미보유 코인의 `pending_target` / 터틀 신호 업데이트
5. **30분 가드 체크** — 진입 신호 목록 생성
...
| `atr_cache.json` | 일봉 기반 지표(ATR·5MA·20MA·10일 신저가) 하루 1회 캐시 |
...
| 항목 | 규칙 | 근거 |
|------|------|------|
| **단일 트레이드 최대 손실** | 자본의 2% | `1 Unit = 총자본 × 1% / ATR`, 손절가 = 진입가 - 2N |
| **코인당 최대 노출** | 4 Unit (≈ 자본의 8%) | `MAX_UNIT_PER_COIN` |
...
- Google Sheets — 체결 원장(워크시트 1) + 포트폴리오 추이(워크시트 2) + 손익차트(워크시트 3)
  ...
  - `run_daily.py` 09:40 에도 동일한 upsert 방식으로 갱신 (중복 없음)
