...
> - 매일 23:55 (`run_daily.py`): `포트폴리오 추이` 시트에 누적 실현손익 1점 기록 + `손익차트` 시트·차트 갱신. 이 잡은 **주문을 내지 않는다**. 이미 당일 매도로 행이 생성된 경우 해당 행을 최신값으로 덮어쓴다(중복 없음). `손익차트` 시트 **K1 셀 드롭다운** (일/주/월/분기/년) 으로 보조 차트의 시간 단위를 사용자가 선택할 수 있다 — 메인(일) 차트는 항상 표시되고, 선택한 단위 차트가 그 아래 추가로 그려진다.  
