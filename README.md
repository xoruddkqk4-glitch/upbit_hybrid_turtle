...
고전 터틀 트레이딩의 **S1/S2 신고가 돌파 + 1시간 시간 가드 안착(마지노선 -0.5ATR 지지)** 진입 필터와 **자금관리 원칙**을 결합한 전략을 구현한다.

> AWS EC2 같은 서버에서 `crontab` 으로 10분마다 `run_all.py` 를 1회씩 호출하는 **원샷 배치** 방식으로 동작한다.
...
- **터틀 S1/S2 + 시간 가드**: 20일(S1) 또는 55일(S2) 신고가 돌파 후 **1시간(60분) 동안 시간 가드**가 작동하며, 가드 시간 도중 `돌파선 - 0.5 * ATR` 마지노선 붕괴 없이 1시간 만료 시점에 `현재가 ≥ 돌파선` 일 때 최종 진입. (타이머 리셋 시 목표가를 `최고점 + 0.2 * ATR`로 상향 갱신하여 횡보장 루프 방지)
- **리스크 균등화 수량**: `1 Unit = 총자본 × 2% / ATR(N)` → 코인별 리스크 동일화
- **0.5N 피라미딩**: 마지막 매수가 대비 0.5N 상승 시마다 1 Unit 추가 (코인당 최대 3 Unit). 추가 매수 시 손절가는 최근 매수가(체결가) 기준으로 갱신.
- **분할 익절**: 평균 매입가 대비 5% 상승 시 보유 수량의 25% 익절(1회), 10% 상승 시 남은 수량의 33% 익절(1회). 평단이 변경되어도 거래 수명 주기 중 각 1회씩만 실행되며, 급등으로 바로 10% 이상으로 가면 33%만 익절하고 두 조건 모두 처리 완료로 간주.
- **고정 2N 손절**: 진입 또는 피라미딩(추가 매수) 시점의 `마지막 매수가 - 2N` 이하 하락 시 전량 즉시 매도. 장중에 가격이 상승하더라도 손절가를 추가로 상향하지 않고 고정하여 자산 보호와 휩쏘 방지를 도모하며, 피라미딩 시에만 최신 매수가 기준으로 갱신됨.
- **트레일링 스탑**: 10일 신저가 경신(매 실행 실시간) 또는 (수익권에서의) **어제 일봉 종가 < 5MA** 시 청산 (후자는 09:06 하루 1회 판정)
- **포트폴리오 상한**: 전체 15 Unit 이내
...
로그인 → 손절·익절 감시 → 터틀 신호 갱신 → 눌림→재돌파 체크 → 주문 순서대로 **1회** 실행하고 종료한다.
...
> **동작 설명**  
> - 매 10분 (`run_all.py`): 손절·익절 감시 → 터틀 신호 갱신 → 눌림→재돌파 체크 → 신규 진입/피라미딩. 일봉 지표는 `atr_cache.json` 캐시에서 읽으므로 일봉 API 를 직접 호출하지 않는다.  
...
> - 매일 23:55 (`run_daily.py`): `포트폴리오 추이` 시트에 누적 실현손익 1점 기록 + `손익차트` 시트·차트 갱신. 이 잡은 **주문을 내지 않는다**. 이미 당일 매도로 행이 생성된 경우 해당 행을 최신값으로 덮어쓴다(중복 없음). `손익차트` 시트 **F1 셀 드롭다운** (일/주/월/분기/년) 을 바꾸면 **차트가 즉시 그 단위로 다시 그려진다** (사전에 숨김 시트 `차트데이터` 에 저장된 5단위 집계를 ARRAYFORMULA 가 동적으로 골라 차트에 공급 — 외부 트리거·Apps Script 불필요).  
...
**터틀 S1 / S2 신고가 돌파 + 눌림→재돌파 (`TURTLE_S1` / `TURTLE_S2`)**

두 조건을 **AND** 로 모두 만족해야 진입한다.

1. 직전 20일(S1) 또는 55일(S2) 장중 고가를 현재가가 돌파 (`turtle_s1/s2_signal = True`)
2. 돌파 후 **한 번 눌렸다가 그 시점의 최고점을 다시 돌파** (`turtle_s1/s2_entry_ready = True`)

**상태 흐름:**
- `WATCHING`: 신호 ON 후 최고값을 계속 갱신. 현재가 < 최고값이 되면 최고값 잠금 → `PULLBACK`
- `PULLBACK`: 최고값 잠금 상태. 현재가 > 잠긴 최고값이면 진입 신호 발생
- 신호가 꺼지면(현재가 < 신고가 기준선) 최고값·상태 전체 초기화
...
- **코인당 최대 3 Unit**, **포트폴리오 전체 15 Unit** 상한
- 피라미딩 시 손절가는 최근 매수가(체결가) 기준으로 갱신되며, 장중 시세 상승에 따른 동적 트레일링 상향 갱신은 수행하지 않고 고정됨

### 청산

| 조건 | 발동 시점 | 비고 |
|------|-----------|------|
| **고정 2N 손절** | 현재가 ≤ `마지막 매수가 - 2N` | 최우선 판정. 진입 또는 피라미딩 시점에 고정 계산된 손절가를 유지하여 장중 휩쏘 방지 |
| **10일 신저가 경신** | 현재가 ≤ 최근 10일 최저 종가 | 무조건 청산 (매 실행 실시간) |
| **5% 분할 익절** | 평단 대비 수익률 ≥ 5.0% 및 < 10.0% | 현재 보유 수량의 25% 시장가 매도. 포지션 수명 주기 중 1회 제한 (`tp_5_done` 플래그) |
| **10% 분할 익절** | 평단 대비 수익률 ≥ 10.0% | 현재 보유 수량의 33% 시장가 매도. 포지션 수명 주기 중 1회 제한 (`tp_10_done` 플래그). 갭상승 등으로 5%를 거치지 않고 바로 10% 돌파 시에도 33%만 매도하며 두 플래그 모두 True 처리 |
| **어제 종가 5MA 하향 돌파** | 어제 일봉 종가(`prev_close`) < 어제까지 5MA(`ma5_prev`) **AND** 현재가 > 평균 매입가 | 수익권에서만 작동. 장중 휴쓰 방지를 위해 09:00 이후 그날 첫 실행(09:06)에만 하루 1회 판정 (`ma5_check_record.json` 가드) |

> 손실권에서는 5MA 이탈로 매도하지 않는다. 2N 하드 손절에만 의존해 추세가 살아날 가능성을 남긴다.
> 5MA 익절의 "추세 꼬임"은 **어제 확정 종가** 기준으로 판단하고(장중 출렉임 무시), **수익권 여부**는 실시간 현재가로 판단한다.
...
└── [SA-MODULE-TRADE] 주문·리스크
    ...
    └── risk_guardian.py      — 2N 손절 + 트레일링 스탑 (5MA 익절은 어제 종가 기준·09:06 하루 1회)
```
...
4. **터틀 신호 갱신** — 미보유 코인의 S1/S2 신호 및 peak(최고값) 상태 업데이트
5. **눌림→재돌파 체크** — 진입 조건 충족 코인을 진입 신호 목록으로 생성
...
| `held_coin_record.json` | 보유 코인의 Unit 수·매수가·평균단가·손절가(고정)·최고가·피라미딩 트리거가·분할익절 완료플래그(`tp_5_done`, `tp_10_done`) |
| `trade_ledger.json` | 누적 체결 원장 |
| `daily_snapshot.json` | `run_daily.py` 의 하루 1회 스냅샷 중복 방지. 매도 즉시 갱신 경로는 건드리지 않음. |
| `atr_cache.json` | 일봉 기반 지표(ATR·5MA·ma5_prev·20MA·10일 신저가·prev_close) 하루 1회 캐시 — `run_cache.py` 가 09:10 에 저장 |
| `ma5_check_record.json` | 5MA 익절 "하루 1회" 가드 (`last_checked_date`). 그날 첫 실행에만 어제 종가 vs 5MA 비교 수행 → 이후 실행은 5MA 익절 스킵 |
...
| **포트폴리오 최대 노출** | 15 Unit (≈ 자본의 30%) | `MAX_TOTAL_UNITS` |
