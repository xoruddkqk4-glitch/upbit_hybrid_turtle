...
- **트레일링 2N 손절**: 매수 후 최고가 - 2N 이하 하락 시 전량 즉시 매도 (가격이 오를수록 손절 기준이 상향되며, 단방향 상향만 보장됨)
...
- 피라미딩 시 손절가는 최근 매수가(체결가) 기준으로 갱신되며, 이후에도 `매수후최고가 - 2N` 트레일링으로 자동 상향 갱신

### 청산

| 조건 | 발동 시점 | 비고 |
|------|-----------|------|
| **트레일링 2N 손절** | 현재가 ≤ `매수후최고가 - 2N` | 최우선 판정. 최고가는 매 실행마다 갱신되며, 손절가는 단방향으로 상향만 됨 |
...
| `held_coin_record.json` | 보유 코인의 Unit 수·매수가·평균단가·손절가(트레일링)·최고가·피라미딩 트리거가·분할익절 완료플래그(`tp_5_done`, `tp_10_done`) |
